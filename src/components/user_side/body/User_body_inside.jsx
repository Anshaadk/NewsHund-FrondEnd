...
function User_body_inside() {
  ...
  const handlePnewsClick = (newsId) => {
    ...
    if (userId) {
      axiosInstance
        .post(`user_side/api/like/${newsId}/${userId}/`)
        .then((response) => {
          if (response.status === 200) {
            setLiked(response.data.is_followed);
          }
        })
        .catch((error) => {
    ...
    <div>
      ...
      <div className='row d-flex justif-content-center border-0'>
        {newsdata?.map((e) => (
          <div
            key={e.id}
            className={`card card_pop col-sm-6 col-12 border-0 col-md-6 col-lg-6 col-xl-3 mb-3 ${
  ...
  {Array.from({ length: Math.floor(Number(e.average_rating)) }, (_, index) => (
    <span key={index} className="star filled-star">★</span>
  ))}
  {Array.from({ length: 5 - Math.floor(Number(e.average_rating)) }, (_, index) => (
    <span key={index + Math.floor(Number(e.average_rating))} className="star empty-star">☆</span>
  ))}
  <span className="average-rating">
    <span className="average-rating-value">{e.average_rating}</span>
  </span>
